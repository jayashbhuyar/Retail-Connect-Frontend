...
const RetailerNetwork = () => {
    ...
    const fetchAcceptedRequests = async () => {
      ...
      try {
        const response = await fetch(
          ` https://retail-connect-backend.onrender.com/api/network/all/accepted/retailer?email=${userEmail}`,
          {
  ...
  const handleMoreInfo = async (request) => {
    try {
      const response = await fetch(
        ` https://retail-connect-backend.onrender.com/api/users/data/${request.distributorEmail}`,
        {
          method: "GET", // Specify the request method if needed (GET is default)
          credentials: "include", // Include credentials with the request
        }
